...
/// MARK: - 타입 캐스팅 Type Casting. 형 변환
/// - 스위프트는 데이터 타입 안정성을 위하여 각기 다른 타입끼리의 값 교환을 엄격하게 제한했다.
/// - 다른 프로그래밍 언어에서 지원하는 암시적 데이터 타입 변환은 지원하지 않는다.

// C 언어에서의 타입캐스팅
//double value = 3.3
//int value2= (int)value
//value2 = 5.5    // double -> int 암시적 데이터 타입 변환
...
/// 스위프트에서의 타입 캐스팅
/// - 스위프트의 타입 캐스팅은 인스턴스의 타입을 확인하거나 자신을 다른 타입의 인스턴스인 양 행세할 수 있는 방법으로 사용할 수 있다.
...
class Americano: Coffee {
    ...
    override var desc: String {
        return "\(shot)샷의 \(iced ? "아이스" : "핫" ) 아메리카노 입니다."
    }
...
/// 데이터 타입 확인
/// is: 데이터 타입을 확인하는 데 사용한다.

let coffee: Coffee = Coffee(shot: 1)
print(coffee.desc)  // 1샷의 커피입니다.
...
// 자식 클래스는 부모 클래스의 모든 멤버를 갖고 있기 때문에 부모 클래스의 객체인 양 행세할 수 없다.
...
/// 다운 캐스팅
/// - 정말로 해당 클래스의 인스턴스를 참조하지 않을 수도 있다. 자식클래스 인스턴스가 부모 클래스 타입의 참조 변수로 참조하고 있을 수 있다.
/// - 부모 클래스 참조 변수에서 자식 클래스 인스턴스를 담고 있을 때
/// 참조 변수를 자식 클래스 타입으로 바꿔야 할 때가 있다. 이 때 다운 캐스팅을 해준다. 자식 멤버에 접근해야 할 때
/// - "as?", "as!", "as
let latte2: Coffee = Latte(flavor: "돌체", shot: 2)
let latte3: Latte = latte2 as! Latte

/// 타입 캐스트 연산자
/// - as? : 타입 캐스팅을 시도해본다. 실패 했을 때는 nil을 반환한다.
/// - as! : 타입 캐스팅을 한다. 실패 시 런타입 오류. 반환 타입이 옵셔널이 아니다.
/// - as : 컴파일러가 상공 확신할 때
