...
// 가변 인스턴스 생성
var sangyoon = Student()
sangyoon.name = "채상윤"
sangyoon.class = "스위프트"
Student.classroom = 7
sangyoon.selfIntroduce()
...
//mutableReference.immutableProperty = 2000
...
// 그러면 궅이 왜 클래스도 var, let 따로 쓰느냐?
// 클래스로 가변 인스턴스 생성하면 참조 바꿇 수 있다. (같은 타입의 다른 인스턴스 가리킬 수 있다)
// 클래스로 불변 인스턴스 생성하면 참조 자체도 못 바꾼다. (프로그램 끝날 때까지 하나의 인스턴스만 가리킬 수 있다.
...
var secondStructureInstance: ValueType = firstStructInstance
// 두 번째 구조체 인스턴스 프로퍼티 값 수정
secondStructureInstance.property = 2

// 두 번쨰 구조체 인스턴스는 첫 번쨰 구조체를 똑같이 복사한
// 별도의 인스턴스이기 때문에
// 두 번째 구조체 인스턴스의 프로퍼티 값을 변경해도
// 첫 번쨰 구조체 인스턴스의 프로퍼티 값에는 영향이 없다.
print("firstStructInstance.property \(firstStructInstance.property)")   // 출력: 1
print("secondStructInstance.property \(secondStructureInstance.property)")  // 출력: 2
...
// var를 참조변수로 만들게되면 다른 인스턴스 참조로 수정할 수 있다.
