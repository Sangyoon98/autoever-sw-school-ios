...
print(sayHello("조기환"))  // 클로저 변수 또는 상수에 담으면 실행 가능
...
// 매개변수로 클로저를 받아 반환겂으로 클로저를 실행하는 함수
...
let calculateAdd = calculate(a: 50, b: 20, method: add)
print("calculateAdd: \(calculateAdd)")

// 두 수를 빼는 역할을 하는 클로저
let subtract: (Int, Int) -> Int = { (a: Int, b: Int) -> Int in
    return a - b
}
let calculateSubtract: Int = calculate(a: 50, b: 20, method: subtract)
print("calculateSubtract: \(calculateSubtract)")

// 두 수를 곱하는 역할을 하는 클로저
let multiply: (Int, Int) -> Int = { (a: Int, b: Int) -> Int in
    return a * b
}
let calculateMultiply: Int = calculate(a: 50, b: 20, method: multiply)
print("calculateMultiply: \(calculateMultiply)")

// 두 수를 나누는 역할을 하는 클로저
let divide: (Int, Int) -> Int = { (a: Int, b: Int) -> Int in
    return a / b
}
let calculateDivide: Int = calculate(a: 50, b: 20, method: divide)
print("calculateDivide: \(calculateDivide)")
...
// 변수(상수)를 미리 선언해놓고 전달하느냐, 함수에 전달인자로 바로 전달하느냐의 차이이다.
func testFunction(a: Int) {
    print("a: \(a)")
}
let closure = 10    // "10"이 클로저라고 생각해보자
testFunction(a: closure)    // 변수로 전달
testFunction(a: 10) // 바로 전달
...
// 클로저 바로 전달하면서 축햑 안한 상태
let calculated2 = calculate2(a: 20, b: 30, method: { (first: Int, second: Int) -> Int in
    return first + second
})

// 축약1: calculate 함수의 매개변수 method의 타입이 "(Int, Int) -> Int"이기 때문에
let calculated3 = calculate2(a: 20, b: 30, method: { (first, second) in
    return first + second
})
// 축약2(단축인자 사용): 클로저의 매개변수 자체를 축약. 순서대로 $0, $1, $2...로 표시
let calculated4 = calculate2(a: 20, b: 30, method: {
    return $0 + $1
})
// 축약3(암시적 반환 표현): 클로저의 본문이 단일 표현식일 경우 return 키워드 생략 가능. 한 줄로도 가능.
let calculated5 = calculate2(a: 20, b: 30, method: { $0 + $1 })

// 후행 클로저
// - 마지막 전달인자가 클로저이면 후행 클로저로 표현 가능
let calculated6 = calculate2(a: 20, b: 30) { $0 + $1 }
